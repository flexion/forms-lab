...
export function createAuthoringRoutes(service: ProjectService): Hono {
  ...
  app.post('/:owner/:slug/edit/:branch/authoring/analyze-criteria', async (c) => {
  ...
  app.post('/:owner/:slug/edit/:branch/authoring/update-criteria', async (c) => {
    const owner = c.req.param('owner')
    const slug = c.req.param('slug')
    const branch = c.req.param('branch')
    const user = c.get('user') as SessionUser | null
  ...
  app.post('/:owner/:slug/edit/:branch/authoring/approve-criteria', async (c) => {
    const owner = c.req.param('owner')
    const slug = c.req.param('slug')
    const branch = c.req.param('branch')
    const user = c.get('user') as SessionUser | null
  ...
  app.post('/:owner/:slug/edit/:branch/authoring/plan-structure', async (c) => {
    ...
    try {
      ...
      const state = view.formSpec && view.spec
        ? {
  ...
  app.post('/:owner/:slug/edit/:branch/authoring/generate-section', async (c) => {
    const owner = c.req.param('owner')
    const slug = c.req.param('slug')
    const branch = c.req.param('branch')
    const user = c.get('user') as SessionUser | null
  ...
  app.post('/:owner/:slug/edit/:branch/authoring/evaluate-section', async (c) => {
    const owner = c.req.param('owner')
    const slug = c.req.param('slug')
    const branch = c.req.param('branch')
    const user = c.get('user') as SessionUser | null
