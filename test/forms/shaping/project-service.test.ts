...
describe('ProjectService — FormSpec mutation', () => {
  ...
  describe('updateFormSpec', () => {
    it('commits a new version of the FormSpec', async () => {
      ...
      const updatedView = await service.getProject('alice', project.slug, alice)
      expect(updatedView.formSpec?.title).toBe('Updated Form Title')
    })
  ...
  describe('getFormSpecHistory', () => {
    it('returns commit history for form.json', async () => {
      const project = await service.createProject(
        'Test Project',
        SAMPLE_PDF,
        alice,
      ...
      // Should have at least 2 commits: initial extraction + update
      expect(history.length).toBeGreaterThanOrEqual(2)
      expect(history[0].message).toContain('Update form title')
      expect(
        history.some((c) => c.message.includes('Extract form specifications')),
      ).toBe(true)
    })

    it('returns empty array if form.json never existed', async () => {
      const project = await service.createProject(
        'Test Project',
        SAMPLE_PDF,
        alice,
      )
      // Don't wait for extraction, query immediately
      const history = await service.getFormSpecHistory('alice', project.slug)
