...
- Name: src/design-system/components/flex-header/conformance.test.ts >> flex-header user menu >> trigger click toggles aria-expanded and panel hidden attribute
- Location: src/design-system/components/flex-header/conformance.test.ts:206:3
...
```ts
  119 | 
  120 |     // Open
  121 |     await menuBtn.click()
  122 |     await expect(page.locator('.flex-header__nav')).toHaveClass(/is-visible/)
  123 | 
  124 |     // Click the overlay (appended to body by JS, position: fixed)
  125 |     const overlay = page.locator('.flex-header__overlay')
  126 |     await expect(overlay).toBeVisible()
  127 |     await overlay.click({ position: { x: 10, y: 10 } })
  128 | 
  129 |     await expect(menuBtn).toHaveAttribute('aria-expanded', 'false')
  130 |     await expect(page.locator('.flex-header__nav')).not.toHaveClass(
  131 |       /is-visible/,
  132 |     )
  133 |   })
  134 | 
